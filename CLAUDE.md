...
**Router in `App.tsx`, route components in `pages/`.** `index.tsx` mounts `App.tsx`, which holds only the language/theme state and the router. There is no router library: `App` reads `window.location.pathname`, strips `VITE_BASE` and the `/zh/` locale prefix into `pathWithoutBase`, then branches on `pathWithoutBase === '/route'` flags. Adding a route means adding such a branch in `App.tsx` (the route harness greps for that exact literal) plus a page module under `pages/` loaded through `lazyPage(...)`, so every route ships as its own chunk behind one `Suspense` boundary. The homepage (`pages/HomePage.tsx`) and `NotFoundPage` are imported eagerly; `/archive/:slug` data lives in `pages/archived-works.ts` (the route harness reads it). Cross-route primitives — `Language`/`Theme` types, `normalizePath`/`joinBasePath`/`resolveAssetPath`, `HeaderControls` — live in `app/shared.tsx`; product-page sibling links in `app/product-siblings.tsx`; a page module must not import another page module (put the shared piece in `app/` or a small data file in `pages/`). `pages/legacy/` holds unrouted old page versions that nothing imports — do not wire them back in. `ProductStorePage`, `PenneysGamePage` and `HomePenneyGame` remain in `components/`.
