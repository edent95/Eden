...
The production client uses the dedicated `eden-tan` endpoint. The new project's
first Artifact Registry repository initially returned `deadline_exceeded`, but
provisioning recovered on 2026-08-19: `gcf-artifacts` now exists and the Function is
ACTIVE. GET returns 200, preflight returns 204, both allow `https://eden-tan.com`,
and the response reports the 100-credit contract plus the migrated leaderboard.
...
After any endpoint change, run `npm run check`, publish through the protected
