...
- Eden-owned Firebase services live in the dedicated `eden-tan` project and must not share a sibling product project's backend.
...
- Firebase project selection and RTDB rules are tracked in `.firebaserc` and `database.rules.json`; `npm run verify:firebase` rejects active configuration that points back to the Poker project.
